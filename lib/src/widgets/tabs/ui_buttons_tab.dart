...
///
/// This widget provides a highly customizable alternative to the standard [TabBar],
/// allowing for different decorations, animations, and layouts for each tab button.
class UIButtonsTab extends StatefulWidget implements PreferredSizeWidget {
  UIButtonsTab({
    super.key,
    required this.tabs,
    this.controller,
  ...
  ///
  /// If [Color] is not provided, `Colors.grey.shade300` is used.
  ...
  /// The [EdgeInsets] used for the outer margin of each tab button.
  ...
  ///
  /// If no value is provided, the default height of 46.0 is used. If height is null,
  /// the height is computed from the default height plus the vertical values of
  /// `contentPadding` and `buttonMargin`.
  final double? height;
  final double? width;
  ...
  UIButtonsTab copyWith({
    ...
    return UIButtonsTab(
      ...
      contentCenter: contentCenter ?? this.contentCenter,
  ...
  @override
  State<UIButtonsTab> createState() => _UIButtonsTabState();
}

class _UIButtonsTabState extends State<UIButtonsTab>
  ...
  void initState() {
    ...
    _animationController = AnimationController(
      vsync: this,
      duration: Duration.zero, // No longer run animation
    );
    ...
    _animationController.addListener(() {
      if (mounted) {
        setState(() {
          // only update UI, avoid full rebuild
        });
      }
    });
  }
  ...
  @override
  void didUpdateWidget(UIButtonsTab oldWidget) {
          ...
          child: Container(
            ...
            child: Row(
              ...
              children: <Widget>[
                ...
                tab.text != null
                    ? UIText(tab.text!, style: textStyle)
                    : (tab.child ?? Container()),
              ],
            ),
          ),
