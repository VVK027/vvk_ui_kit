...
class _ImageShimmer extends StatelessWidget {
  const _ImageShimmer({this.width, this.height, this.shape});
  final double? width;
  final double? height;
...
class UIShimmerBase {
  /// Creates a rectangular shimmer, typically for images.
  static Widget image({double? width, double? height}) {
    return _ImageShimmer(width: width, height: height);
  }

  /// Creates a circular shimmer, typically for avatars.
  static Widget atv({double? size}) {
    return _ImageShimmer(width: size, height: size, shape: BoxShape.circle);
  }

  /// Creates a multi-line title shimmer.
  static Widget title({
