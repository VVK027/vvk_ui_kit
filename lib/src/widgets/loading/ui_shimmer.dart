...
// Shimmer animation adapted from the shimmer package (https://pub.dev/packages/shimmer).
// Consider adding `shimmer` as a dependency and removing this vendored copy before publishing.
...
///
/// * use one `UIShimmer` to wrap list of [Widget]s instead of a list of many `UIShimmer`s
///
@immutable
class UIShimmer extends StatefulWidget {
  final Widget child;
  ...
  /// Factory that builds a `UIShimmer` whose [gradient] is a [LinearGradient] made up of `baseColor` and
  /// `highlightColor`.
  ///
  UIShimmer.fromColors({
    super.key,
    required this.child,
  ...
  @override
  State<UIShimmer> createState() => _UIShimmerState();
...
class _UIShimmerState extends State<UIShimmer>
  ...
  @override
  void didUpdateWidget(UIShimmer oldWidget) {
    if (widget.enabled) {
      _controller.forward();
    } else {
      _controller.stop();
